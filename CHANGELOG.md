...
## Promptee-V2.0 (2026-09-21) — 多服务商版本

### 新增功能
- **多服务商支持** — 新增「服务商」选择：火山方舟 ARK / 自定义（OpenAI 兼容），可接入任意兼容 /chat/completions 的服务商
- **配置档案** — 多套 API 配置可命名、并存与一键切换，方舟与自定义各自独立保存
- **配置导入导出** — 导出全部档案为 JSON，可跨机器迁移；导入支持追加或替换
- **API 配置弹窗** — 配置移到右上角齿轮入口，标题栏显示当前配置名，未配置时显示提示点
- **通用代理** — 代理不再写死方舟：域名白名单、内网地址拦截（防 SSRF）、支持 Authorization Bearer / x-api-key / api-key 三种鉴权、按来源 IP 限流
- **测试连接** — 用与真实生成完全相同的链路发送最小请求，回显目标地址、HTTP 状态与错误原文
- **结果持久化** — 图片存入 IndexedDB、文本存入 localStorage，刷新不再丢失，也不再受 5MB 配额限制
- **Excel 导出带图片预览** — 改用 ExcelJS 将缩略图嵌入 xlsx，双图融合结果并排显示主体图与环境图
- **双图融合批量模式** — 支持 1+1 单组、1 主体 + N 环境、N 主体 + 1 环境
- **接入点与参数自填** — 移除三套写死的豆包模型预设，接入点 ID、输出上限、超时、关闭深度思考均可在界面配置

### 修复
- 整体穿搭模式下提示词反向要求描述全部服装，导致输出仍描述原穿搭 — 已改为只描述未替换的服饰
- 融合结果 JSON 中动作姿态重复输出两次 — 已去重
- 配饰遮挡商品的问题 — 补充规则：包袋一律忽略，遮挡商品则移除
- 环境色彩分析混入人物衣着颜色 — 已排除人物及穿搭描述
- 融合入口缺少 API 配置校验，空 Key 会直接发出请求并报 401 — 已补校验
- 模型下拉出现空白 — 对存档中的非法值做归一化
- 头部按钮图标与文字未垂直居中
- 弹窗内下拉选项白字白底不可见

### 优化
- 删除三处重复的函数定义（startFusion / processDualImage / rR），减少约 8KB 冗余代码
- 全身提示词模板更新，补充完整全身景、姿态约束、替换要求、自然融合与负面提示词
- 左右面板比例调整为 24% / 48% / 28%
- 左栏拆分为固定操作区与滚动设置区

### 已知问题
1. 代理受 Vercel 函数约 4.5MB 请求体上限限制，超大图需开启图片压缩
2. 代理限流为进程内存态，非严格配额
3. 白名单外的服务商需配置 `PROXY_ALLOWED_HOSTS` 环境变量后才能走代理
4. 导出的配置为明文 JSON，内含 API Key，请勿公开分享
5. file:// 协议下无法调用 API，需通过本地服务器或部署访问
6. 内置提示词模板不可在界面中直接编辑

---
## Promptee-V1.3 (2026-07-10) — 更新版

### 修复
- **全选功能修复** — 改为直接操作 DOM 元素 class，不重新渲染网格
- **下拉选项颜色修复** — 左面板 select 选项文字颜色与背景区分
- **CSS overflow 修复** — 确保下拉框不被父容器裁剪
